<div class="row padding-1 p-1">
    <div class="col-md-12">
        ...
        <div class="form-group mb-2 mb20">
            <label for="tip_ev" class="form-label">{{ __('Categoría') }}</label>
            <select name="tip_ev" class="form-control @error('tip_ev') is-invalid @enderror" id="tip_ev">
                ...
                <option value="CEREMONIO" @if(old('tip_ev', $evento?->tip_ev) == 'CEREMONIA') selected @endif>CEREMONIA</option>
            </select>
            {!! $errors->first('tip_ev', '<div class="invalid-feedback" role="alert"><strong>:message</strong></div>') !!}
        </div>
        <div class="form-group mb-2 mb20">
            <label for="f_in_ev" class="form-label">{{ __('Fecha de inicio del evento') }}</label>
            <input type="date" name="f_in_ev" class="form-control @error('f_in_ev') is-invalid @enderror" value="{{ old('f_in_ev', optional($evento)->f_in_ev) }}" id="f_in_ev">
            @error('f_in_ev')
        ...
        <div class="form-group mb-2 mb20">
            <label for="nom_ev" class="form-label">{{ __('Nombre del evento') }}</label>
        ...
        <div class="form-group mb-2 mb20">
            <label for="sed_ev" class="form-label">{{ __('Sede del evento') }}</label>
            <select name="sed_ev" class="form-control @error('sed_ev') is-invalid @enderror" id="sed_ev">
                ...
                <option value="Virtual" @if(old('sed_ev', $evento?->sed_ev) == 'Virtual') selected @endif>Virtual</option>
            </select>
            {!! $errors->first('sed_ev', '<div class="invalid-feedback" role="alert"><strong>:message</strong></div>') !!}
        </div>
        <div class="form-group mb-2 mb20">
            <label for="f_fi_ev" class="form-label">{{ __('Fecha de finalización del evento') }}</label>
            <input type="date" name="f_fi_ev" class="form-control @error('f_fi_ev') is-invalid @enderror" value="{{ old('f_fi_ev', optional($evento)->f_fi_ev) }}" id="f_fi_ev">
            @error('f_fi_ev')
                <div class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </div>
