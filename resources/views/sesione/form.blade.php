<div class="row padding-1 p-1">
    <div class="col-md-12">
        ...
        <div class="form-group mb-2 mb20">
            ...
            <select name="eventos_id" class="form-control @error('eventos_id') is-invalid @enderror" id="eventos_id">
                @foreach($evento as $id => $nom_ev)
                    <option value="{{ $id }}" @if(old('evento_id', optional($sesione)->eventos_id) == $id) selected @endif>{{ $nom_ev }}</option>
                @endforeach
            </select>
            {!! $errors->first('eventos_id', '<div class="invalid-feedback" role="alert"><strong>:message</strong></div>') !!}
        </div>
        <div class="form-group mb-2 mb20">
            <label for="t_s" class="form-label">{{ __('Tema de la sesion') }}</label>
        ...
        <div class="form-group mb-2 mb20">
            <label for="cant_s" class="form-label">{{ __('Cantidad estimada de asistentes') }}</label>
            <input type="text" name="cant_s" class="form-control @error('cant_s') is-invalid @enderror" value="{{ old('cant_s', $sesione?->cant_s) }}" id="cant_s" placeholder="Cant S" type="number" required>
